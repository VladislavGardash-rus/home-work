...
func TestRunCmd(t *testing.T) {
	t.Run("Success execute command", func(t *testing.T) {
		...
		err = os.Mkdir(filepath.Join(dir, "envs"), 0777) //nolint:all
		require.NoError(t, err)

		err = ioutil.WriteFile(filepath.Join(dir, "envs", "BAR"), []byte("bar"), 0666) //nolint:all
		require.NoError(t, err)

		err = ioutil.WriteFile(filepath.Join(dir, "time_test.sh"), []byte("#!/usr/bin/env bash\n\necho -e \"BAR is (${BAR})\narguments are $*\""), 0666) //nolint:all
		require.NoError(t, err)

		err = os.Chmod(filepath.Join(dir, "time_test.sh"), 0777) //nolint:all
		require.NoError(t, err)
